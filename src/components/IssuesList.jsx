...
const fetchIssues = async (signal, labels, status, pageNum, queryClient) => {
...
export default function IssuesList({labels, status, pageNum, setPageNum}) {
    ...
    const issuesQuery = useQuery(["issues", {
        ...
        status,
        pageNum
    }], ({signal}) => fetchIssues(signal, labels, status, pageNum, queryClient), {keepPreviousData: true});
    ...
    useEffect(() => {
        queryClient.prefetchQuery(["issues", {
            ...
            status,
            pageNum: pageNum + 1
        }], ({signal}) => fetchIssues(signal, labels, status, pageNum + 1, queryClient))
    }, [pageNum])
